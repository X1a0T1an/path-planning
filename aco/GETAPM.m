...
attractivePotentialScaling=1000; % scaling factor for attractive potential
...
while ~pathFound
    if nt<length(xpos)
        ...
        currentPosition1=[xpos(end),ypos(end),zpos(end)];
    end
    if nt<length(xpos2)
        currentPosition2=[xpos2(nt),ypos2(nt),zpos2(nt)];
    else
        currentPosition2=[xpos2(end),ypos2(end),zpos2(end)];
    end
    if nt<length(xpos3)
        currentPosition3=[xpos3(nt),ypos3(nt),zpos3(nt)];
    else
        currentPosition3=[xpos3(end),ypos3(end),zpos3(end)];
    end
    nt=nt+1;
    ...
    while i<20
        ...
        if dnear==1
            if ~(flag==1)&&sqrt(sum(x-(currentPosition1)).^2)<1;i=7;break;end
            if ~(flag==2)&&sqrt(sum(x-(currentPosition2)).^2)<1;i=7;break;end
            if ~(flag==3)&&sqrt(sum(x-(currentPosition3)).^2)<1;i=7;break;end
        end
        i=i+1;
    end
...
%      if ~feasiblePoint3d(currentPosition)
...
%       h1 = drawSphere(0,0.5,currentPosition(1), currentPosition(2), currentPosition(3));
     % plotting robot
end
end
